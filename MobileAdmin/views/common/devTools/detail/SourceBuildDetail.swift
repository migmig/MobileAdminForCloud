...
struct SourceBuildDetail: View {
            ...
            Section("기능") {
                Button(action: {
                    Task {
                        ...
                        sourceBuildInfoResult = await buildViewModel.fetchSourceBuildInfo(selectedProject?.id ?? 0)?.result
                ...
                .confirmationDialog("실행확인", isPresented: $isConfirm) {
                    Button(action: { runBuild() }) { Text("실행확인") }
                }
            }
            Section("소스") {
    ...
    func getBuildHistory() {
        Task {
            let response = await buildViewModel.fetchSourceBuildHistory(selectedProject?.id ?? 0)
    ...
    func runBuild() {
        Task {
            withAnimation { isLoaded = true }
            let res = await buildViewModel.execSourceBuild(selectedProject?.id ?? 0)
...
#Preview {
    NavigationStack {
        SourceBuildDetail(selectedProject: SourceBuildProject(3334, "oauth-resource-server", "Permission 1", "Action 1"))
