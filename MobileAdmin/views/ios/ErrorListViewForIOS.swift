...
struct ErrorListViewForIOS: View {
    ...
    var filteredErrorItems: [ErrorCloudItem] {
        guard !searchText.isEmpty else { return errorViewModel.errorItems }
                ...
                Section {
                    HStack(spacing: AppSpacing.sm) {
                        ...
                        if !searchText.isEmpty {
                            Text("(전체 \(errorViewModel.errorItems.count)건)")
    ...
    private func triggerUserLogDownload() {
        ...
        Task {
            do {
                downloadedFileURL = try await errorViewModel.downloadUserLog(userIdForLog)
            } catch {
                downloadedFileURL = nil
            }
